...
function make_invoice_pdf(int $orderId): string {
    ...
    $content = '';
    // Premium AZARO stationery: teal + deep charcoal + generous whitespace.
    $teal = '0 0.592 0.698';
    $dark = '0.035 0.12 0.15';
    $muted = '0.35 0.40 0.42';
    $light = '0.95 0.97 0.97';

    $content .= "1 1 1 rg\n0 0 595 842 re f\n";
    $content .= $teal . " rg\n0 704 595 138 re f\n";
    $content .= $dark . " rg\n365 704 230 138 re f\n";
    ...
    $meta = [
        ...
        ['MOBILE', $order['buyer_phone'] ?? 'N/A'],
        ['EMAIL', $order['buyer_email'] ?? ''],
    ];
    $metaX = [58, 205, 352, 450];
    foreach ($meta as $i => $pair) {
        $x = $metaX[$i];
        $value = substr((string)$pair[1], 0, $i === 3 ? 23 : 24);
        $content .= $muted . " rg\nBT\n/F1 7 Tf\n1 0 0 1 {$x} 650 Tm\n(" . pdf_escape($pair[0]) . ") Tj\nET\n";
        $content .= $dark . " rg\nBT\n/F1 9 Tf\n1 0 0 1 {$x} 632 Tm\n(" . pdf_escape($value) . ") Tj\nET\n";
    }

    // Delivery address.
    $content .= $muted . " rg\nBT\n/F1 7 Tf\n1 0 0 1 45 578 Tm\n(" . pdf_escape('DELIVERY ADDRESS') . ") Tj\nET\n";
    $address = trim((string)($order['address'] ?? '')) ?: 'N/A';
    $address = substr($address, 0, 88);
    $content .= $dark . " rg\nBT\n/F1 10 Tf\n1 0 0 1 45 559 Tm\n(" . pdf_escape($address) . ") Tj\nET\n";

    // Item table.
    $tableY = 500;
    $content .= $dark . " rg\n45 {$tableY} 505 31 re f\n1 1 1 rg\n";
    foreach ([[58,'ITEM'],[360,'QTY'],[415,'UNIT PRICE'],[500,'TOTAL']] as $h) {
    ...
    foreach ($items as $item) {
        ...
        $content .= $dark . " rg\n";
        $name = @iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', (string)$item['name']);
        if ($name === false) $name = (string)$item['name'];
        $name = substr($name, 0, 43);
        $row = [
            [58, $name],
            [362, number_format($qty, 0)],
            [415, number_format($unit, 2)],
            [500, number_format($line, 2)]
        ];
        foreach ($row as $r) {
            $content .= "BT\n/F1 8 Tf\n1 0 0 1 {$r[0]} " . ($y + 9) . " Tm\n(" . pdf_escape($r[1]) . ") Tj\nET\n";
        }
    ...
    $discount = max(0, $subtotal - $grand);
    $totalY = max(190, $y - 18);

    // Totals block.
    $content .= $light . " rg\n350 {$totalY} 200 98 re f\n";
    $content .= $muted . " rg\nBT\n/F1 9 Tf\n1 0 0 1 365 " . ($totalY + 72) . " Tm\n(" . pdf_escape('SUBTOTAL') . ") Tj\nET\n";
    $content .= $dark . " rg\nBT\n/F1 9 Tf\n1 0 0 1 480 " . ($totalY + 72) . " Tm\n(" . pdf_escape(number_format($subtotal,2)) . ") Tj\nET\n";
    if ($discount > 0) {
        $content .= $muted . " rg\nBT\n/F1 9 Tf\n1 0 0 1 365 " . ($totalY + 50) . " Tm\n(" . pdf_escape('DISCOUNT') . ") Tj\nET\n";
        $content .= $dark . " rg\nBT\n/F1 9 Tf\n1 0 0 1 480 " . ($totalY + 50) . " Tm\n(" . pdf_escape('- ' . number_format($discount,2)) . ") Tj\nET\n";
    }
    $content .= $teal . " rg\nBT\n/F1 13 Tf\n1 0 0 1 365 " . ($totalY + 22) . " Tm\n(" . pdf_escape('TOTAL') . ") Tj\nET\n";
    $content .= "BT\n/F1 13 Tf\n1 0 0 1 445 " . ($totalY + 22) . " Tm\n(" . pdf_escape('BDT ' . number_format($grand,2)) . ") Tj\nET\n";

    // Status strip and delivery information.
    $status = strtoupper((string)($order['status'] ?? 'pending'));
    $courier = (string)($order['courier_status'] ?? 'Not sent');
    $content .= $teal . " rg\n45 " . ($totalY + 20) . " 280 36 re f\n1 1 1 rg\n";
    $content .= "BT\n/F1 8 Tf\n1 0 0 1 58 " . ($totalY + 34) . " Tm\n(" . pdf_escape('ORDER STATUS: ' . $status) . ") Tj\nET\n";
    $content .= "BT\n/F1 8 Tf\n1 0 0 1 58 " . ($totalY + 22) . " Tm\n(" . pdf_escape('COURIER: ' . $courier) . ") Tj\nET\n";
