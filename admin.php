...
$recentOrders=$pdo->query("SELECT o.*,u.name buyer_name,u.email buyer_email,u.phone buyer_phone FROM orders o JOIN users u ON u.id=o.client_id ORDER BY o.date DESC LIMIT 10")->fetchAll();
...
// Orders page filter: pending = incoming, Sent to courier = courier, Delivered = delivered, Returned = returned.
$orderFilter=$_GET['order_filter']??'all';
$allowedOrderFilters=['all','incoming','courier','delivered','returned'];
if(!in_array($orderFilter,$allowedOrderFilters,true))$orderFilter='all';
$orderWhere='';$orderParams=[];
if($orderFilter==='incoming'){$orderWhere=" WHERE o.status='pending'";}
elseif($orderFilter==='courier'){$orderWhere=" WHERE o.courier_status='Sent to courier'";}
elseif($orderFilter==='delivered'){$orderWhere=" WHERE (o.courier_status='Delivered' OR (o.status='delivered' AND o.courier_status<>'Returned'))";}
elseif($orderFilter==='returned'){$orderWhere=" WHERE o.courier_status='Returned'";}
$allStmt=$pdo->prepare("SELECT o.*,u.name buyer_name,u.email buyer_email,u.phone buyer_phone FROM orders o JOIN users u ON u.id=o.client_id".$orderWhere." ORDER BY o.date DESC");
$allStmt->execute($orderParams);$allOrders=$allStmt->fetchAll();
...
<div class="admin-shell">
  ...
  <main class="admin-main">
    ...
    <?php if($section==='dashboard'): ?>
      ...
      <div class="admin-grid-2"><section class="admin-card"><div class="admin-card-head"><div><span class="admin-kicker">RECENT ACTIVITY</span><h2>Recent Orders</h2></div><a href="admin.php?section=orders">View all</a></div><div class="order-list"><?php foreach($recentOrders as $o):?><div class="order-row"><div class="order-icon">#<?=e($o['id'])?></div><div class="order-main"><b><?=e($o['buyer_name'])?></b><span><?=e($o['buyer_phone'] ?: 'No mobile number')?></span></div><div class="order-money"><?=money($o['price'])?></div><span class="status-pill status-<?=e($o['status'])?>"><?=e(ucfirst($o['status']))?></span></div><?php endforeach;?><?php if(!$recentOrders):?><p class="muted">No orders yet.</p><?php endif;?></div></section><section class="admin-card"><div class="admin-card-head"><div><span class="admin-kicker">STORE HEALTH</span><h2>Quick Actions</h2></div></div><div class="quick-grid"><a href="admin.php?section=products"><b>＋</b><span>Add product</span></a><a href="admin.php?section=categories"><b>◇</b><span>Manage categories</span></a><a href="admin.php?section=customers"><b>♙</b><span>Customers</span></a><a href="admin.php?section=reports"><b>▥</b><span>Sales reports</span></a></div></section></div>
    <?php elseif($section==='orders'): ?>
      <section class="admin-card"><div class="admin-card-head"><div><span class="admin-kicker">ORDER MANAGEMENT</span><h2>Orders</h2></div><div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap"><form method="get" class="order-filter-form"><input type="hidden" name="section" value="orders"><label class="sr-only" for="order-filter">Filter orders</label><select id="order-filter" name="order_filter" class="status-select" onchange="this.form.submit()"><option value="all" <?=$orderFilter==='all'?'selected':''?>>All Orders</option><option value="incoming" <?=$orderFilter==='incoming'?'selected':''?>>Incoming</option><option value="courier" <?=$orderFilter==='courier'?'selected':''?>>Sent to Courier</option><option value="delivered" <?=$orderFilter==='delivered'?'selected':''?>>Delivered</option><option value="returned" <?=$orderFilter==='returned'?'selected':''?>>Returned</option></select></form><span class="count-badge"><?=count($allOrders)?> shown</span></div></div><div class="table-scroll"><table class="admin-table"><thead><tr><th>Order</th><th>Buyer</th><th>Address</th><th>Amount</th><th>Status</th><th>Courier</th><th>Action</th></tr></thead><tbody><?php foreach($allOrders as $o):?><tr><td><b>#<?=e($o['id'])?></b><small><?=e($o['date'])?></small></td><td><b><?=e($o['buyer_name'])?></b><small><?=e($o['buyer_phone'] ?: 'No mobile number')?></small></td><td><?=e($o['address']??'N/A')?></td><td><b><?=money($o['price'])?></b></td><td><?php if($o['status']==='pending'):?><form method="post" class="inline-form"><?=csrf_field()?><input type="hidden" name="action" value="confirm_order"><input type="hidden" name="order_id" value="<?=$o['id']?>"><button class="status-button pending-button">Confirm</button></form><?php else:?><form method="post" class="inline-form"><?=csrf_field()?><input type="hidden" name="action" value="order_status"><input type="hidden" name="order_id" value="<?=$o['id']?>"><select name="status" onchange="this.form.submit()" class="status-select"><option value="pending" <?=$o['status']==='pending'?'selected':''?>>Pending</option><option value="confirmed" <?=$o['status']==='confirmed'?'selected':''?>>Confirmed</option><option value="delivered" <?=$o['status']==='delivered'?'selected':''?>>Delivered</option><option value="cancelled" <?=$o['status']==='cancelled'?'selected':''?>>Cancelled</option></select></form><?php endif;?></td><td><form method="post"><?=csrf_field()?><input type="hidden" name="action" value="courier_status"><input type="hidden" name="order_id" value="<?=$o['id']?>"><select name="courier_status" onchange="this.form.submit()" class="status-select"><option>Not sent</option><option <?=$o['courier_status']==='Sent to courier'?'selected':''?>>Sent to courier</option><option <?=$o['courier_status']==='Delivered'?'selected':''?>>Delivered</option><option <?=$o['courier_status']==='Returned'?'selected':''?>>Returned</option></select></form></td><td><a class="table-action" href="<?=BASE_URL?>/invoice.php?id=<?=$o['id']?>">Invoice</a></td></tr><?php endforeach;?><?php if(!$allOrders):?><tr><td colspan="7" style="text-align:center;padding:40px" class="muted">No orders match this filter.</td></tr><?php endif;?></tbody></table></div></section>
